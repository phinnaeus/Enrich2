...
class Configurator(tk.Tk):
    ...
    def create_main_frame(self):
        ...
        for i, k in enumerate(SCORING_METHODS.keys()):
            rb = tkinter.ttk.Radiobutton(options_frame, text=SCORING_METHODS[k].title(), variable=self.scoring_method, value=k)
        ...
        for i, k in enumerate(LOGR_METHODS.keys()):
            rb = tkinter.ttk.Radiobutton(options_frame, text=LOGR_METHODS[k].title(), variable=self.logr_method, value=k)
            rb.grid(column=0, row=row, sticky="w")
            row += 1
            if i == 0:
                rb.invoke()
